...
switch type_Activation
    ...
    case 11 %  Mish 
        ...
        act = (b + w*x)*(( exp((log(exp(b + w*x) + 1))) - exp(-(log(exp(b + w*x) + 1))))/ ( ( exp((log(exp(b + w*x) + 1))) + exp(-(log(exp(b + w*x) + 1)))) ))   ;

        
        actd =((b + w*x)*(w*exp(b + w*x) + (w*exp(b + w*x))/(exp(b + w*x) + 1)^2))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1) + (w*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1) - ((b + w*x)*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^2;
 
 
        actdd =((b + w*x)*(w^2*exp(b + w*x) - (2*w^2*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^3 + (w^2*exp(b + w*x))/(exp(b + w*x) + 1)^2))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1) + (2*w*(w*exp(b + w*x) + (w*exp(b + w*x))/(exp(b + w*x) + 1)^2))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1) - (2*(b + w*x)*(w*exp(b + w*x) + (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^2 - ((b + w*x)*(w^2*exp(b + w*x) + (2*w^2*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^3 - (w^2*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^2 + (2*(b + w*x)*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)^2*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^3 - (2*w*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^2;
 

        actddd =(3*w*(w^2*exp(b + w*x) - (2*w^2*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^3 + (w^2*exp(b + w*x))/(exp(b + w*x) + 1)^2))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1) + ((b + w*x)*(w^3*exp(b + w*x) - (6*w^3*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^3 + (w^3*exp(b + w*x))/(exp(b + w*x) + 1)^2 + (6*w^3*exp(b + w*x)*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^4))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1) - ((b + w*x)*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1)*(w^3*exp(b + w*x) + (6*w^3*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^3 - (w^3*exp(b + w*x))/(exp(b + w*x) + 1)^2 - (6*w^3*exp(b + w*x)*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^4))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^2 - (3*(b + w*x)*(w*exp(b + w*x) + (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(w^2*exp(b + w*x) + (2*w^2*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^3 - (w^2*exp(b + w*x))/(exp(b + w*x) + 1)^2))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^2 - (3*(b + w*x)*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(w^2*exp(b + w*x) - (2*w^2*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^3 + (w^2*exp(b + w*x))/(exp(b + w*x) + 1)^2))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^2 + (6*w*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)^2*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^3 + (6*(b + w*x)*(w*exp(b + w*x) + (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)^2)/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^3 - (6*w*(w*exp(b + w*x) + (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^2 - (6*(b + w*x)*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)^3*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^4 - (3*w*(w^2*exp(b + w*x) + (2*w^2*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^3 - (w^2*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^2 + (6*(b + w*x)*(w*exp(b + w*x) - (w*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(w^2*exp(b + w*x) + (2*w^2*exp(2*b + 2*w*x))/(exp(b + w*x) + 1)^3 - (w^2*exp(b + w*x))/(exp(b + w*x) + 1)^2)*(exp(b + w*x) - 1/(exp(b + w*x) + 1) + 1))/(exp(b + w*x) + 1/(exp(b + w*x) + 1) + 1)^3;
        
     case 12   % Swish
        
         act = (b + w*x)*(1/(exp(- b - w*x) + 1));
         
         actd = w/(exp(- b - w*x) + 1) + (w*exp(- b - w*x)*(b + w*x))/(exp(- b - w*x) + 1)^2;
 
 
         actdd =(2*w^2*exp(- b - w*x))/(exp(- b - w*x) + 1)^2 - (w^2*exp(- b - w*x)*(b + w*x))/(exp(- b - w*x) + 1)^2 + (2*w^2*exp(- 2*b - 2*w*x)*(b + w*x))/(exp(- b - w*x) + 1)^3;
 
 
         actddd =(6*w^3*exp(- 2*b - 2*w*x))/(exp(- b - w*x) + 1)^3 - (3*w^3*exp(- b - w*x))/(exp(- b - w*x) + 1)^2 + (w^3*exp(- b - w*x)*(b + w*x))/(exp(- b - w*x) + 1)^2 - (6*w^3*exp(- 2*b - 2*w*x)*(b + w*x))/(exp(- b - w*x) + 1)^3 + (6*w^3*exp(- b - w*x)*exp(- 2*b - 2*w*x)*(b + w*x))/(exp(- b - w*x) + 1)^4;
